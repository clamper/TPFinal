$(document).ready(function () {
    ...
    $("#show_detail_content").on("click", "#open_add_cart", function () {

        $("input[name=idpresentation]").val($(this).attr("data"));
        $("#modal_add_location_to_cart").modal('show');
    });

    $("#modal_add_location_to_cart").on("change"
        , "#location_cant", function () {
            ...
            var total = 0;
            $("span[id^=total]").each(function (index, element) {
                total = parseInt(total) + parseInt($(this).html().substr(1, 7));
            });

            $("#total").html("total $" + total);
        });

    $("#final_buy").on("click", function (event) {
        $("#formAddCart").submit();
    });
